...
رابط کاربری فارسی و راست‌به‌چپ است. با دکمه‌ی **EN** در نوار بالای برنامه می‌توانید به انگلیسی سوئیچ کنید: صفحه چپ‌به‌راست می‌شود، تاریخ‌ها میلادی و رقم‌ها لاتین نمایش داده می‌شوند. متن‌ها از فایل [src/web/i18n.json](src/web/i18n.json) ترجمه می‌شوند؛ هر رشته‌ای که هنوز ترجمه نشده باشد فارسی می‌ماند و برای تکمیل ترجمه کافی است به همان فایل اضافه شود.
...
برای امتحان کردن برنامه یا گرفتن خروجی نمایشی، فایل [sql/demo_data.sql](sql/demo_data.sql) (فارسی) یا [sql/demo_data_en.sql](sql/demo_data_en.sql) (انگلیسی) را روی یک دیتابیس **خالی** که برنامه یک بار روی آن اجرا شده اجرا کنید. حدود ۳۰۰ ردیف داده‌ی ساختگی اضافه می‌کند: کاربر با نقش‌های مختلف، پروژه، تسک در همه‌ی وضعیت‌ها، حضور و غیاب، مرخصی، قرارداد و صورت‌وضعیت، برنامه‌ی مالی و امتیاز و فروشگاه. رمز همه‌ی کاربران نمونه `Taskhub@1404` است.

اگر دیتابیس از قبل داده داشته باشد اسکریپت اجرا نمی‌شود، و اگر وسط کار به خطا بخورد همه‌چیز را برمی‌گرداند.
